...
class EmailNotifier(NotifierBase):
    ...
    def notify_new(self, items):
        ...
        for item in items:
            ...
            if item.image_link:
                data['image'] = u'<a href="{}"> <img src="{}" width=auto height=120 >'.format(item.link, item.image_link)
    ...
    def notify_special_items(self, items):
        ...
        for item in items:
            data = item.to_json(summarize=True)
            if item.image_link:
                data['image'] = u'<a href="{}"> <img src="{}" width=auto height=120 > </a>'.format(item.link, item.image_link)
            html_doc = json2html.convert(json=data).replace('<li>', '').replace('</li>', '').replace('<ul>', '').replace('</ul>', '')
            body += u'{}<br> </br>'.format(html_doc)
